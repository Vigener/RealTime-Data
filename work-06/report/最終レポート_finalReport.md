...
- [5. 技術的工夫と課題解決](#5-技術的工夫と課題解決)
  - [5.1 データ整合性の保証](#51-データ整合性の保証)
    ...
    - [5.1.3 空売り防止機能(ロジック不十分)](#513-空売り防止機能ロジック不十分)
...
- [6. 開発過程で直面した課題](#6-開発過程で直面した課題)
  - [6.1 技術的課題](#61-技術的課題)
    - [6.1.1 データ同期問題](#611-データ同期問題)
    - [6.1.2 マイクロサービス間通信の複雑性](#612-マイクロサービス間通信の複雑性)
    - [6.1.3 リアルタイム性能の確保](#613-リアルタイム性能の確保)
  - [6.2 課題への対処法](#62-課題への対処法)
    - [6.2.1 実装した解決策](#621-実装した解決策)
  - [6.3 未解決の課題](#63-未解決の課題)
- [7. 評価と検証・考察](#7-評価と検証考察)
  - [7.1 機能評価](#71-機能評価)
    - [7.1.1 実装した機能の動作確認](#711-実装した機能の動作確認)
    - [7.1.2 パフォーマンス測定](#712-パフォーマンス測定)
  - [7.3 システムの限界と制約](#73-システムの限界と制約)
  - [7.4 リアルタイムデータ処理技術の比較](#74-リアルタイムデータ処理技術の比較)
- [8. 今後の展望](#8-今後の展望)
  - [8.1 短期的改善案](#81-短期的改善案)
    - [8.1.1 データ同期問題の根本解決](#811-データ同期問題の根本解決)
    - [8.1.2 投資家行動の高度化](#812-投資家行動の高度化)
  - [8.2 長期的発展構想](#82-長期的発展構想)
    - [8.2.1 機能拡張（信用取引、配当システム等）](#821-機能拡張信用取引配当システム等)
- [9. まとめ](#9-まとめ)
  - [9.1 達成した成果](#91-達成した成果)
  - [9.2 技術的な学び](#92-技術的な学び)
  - [9.3 今後の課題](#93-今後の課題)
- [10. 参考文献・使用技術](#10-参考文献使用技術)
- [付録](#付録)
...
本システムの開発にあたり、以下の技術要件と制約条件を設定した：
...
株主情報は`ShareholderInfo`クラスで表現され、以下の属性を持つ：
...
取引データは`Transaction`クラスと`TransactionData`クラスで表現され、以下の構造を持つ：
...
ポートフォリオは`Portfolio`クラスと`Portfolio.Entry`クラスによって表現されている：
...
また、地域別の集計データは`RegionSummary`クラスで管理される：
...
React Bootstrapを活用して、画面サイズに応じて３段階のレイアウトを動的に切り替えるレスポンシブデザインを実装した。
...
| --- | --- | --- |
| 1列縦並び | 2列表示 | 3列表示 |
| ![alt text](images/image-24.png) | ![alt text](images/image-26.png) | ![alt text](images/image-27.png) |
...
実際の市場では、取引量や取引方向（買いか売りか）に応じて株価が変動する。本システムでは、よりリアルな市場シミュレーションを実現するために、取引に応じた動的価格変動アルゴリズムを実装した。[価格変動のコード](#価格変動ロジック)
...
現実の投資家行動をシミュレートするため、保有株数に基づいた売買判断ロジックを実装した。[保有株数ベースの売買量決定コード](#売買数量決定ロジック)
...
1. **双方向通信の難しさ**:
   ...
   この構造で、StockProcessorの接続状態をTransactionに伝えるためには、PriceManagerを介した双方向通信が必要であった。しかし、この実装は、〆切直前の仕様変更だったこともあり、時間的制約の中で完全には実装できなかった。
...
**データ量と更新頻度のトレードオフ**:
   ...
   後述の[7.1.2 パフォーマンス測定](#712-パフォーマンス測定)で詳しく検証する。
...
本システムで使用したリアルタイムデータ処理技術と、他の一般的な技術を比較する：
...
これらの改善により、各サービス間でのデータ整合性が確保され、マイナス保有株数などの問題が解消されると考えられる。
