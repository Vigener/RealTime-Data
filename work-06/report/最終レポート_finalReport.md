...
1. **初期状態同期メカニズム**:
   ...
   public void sendInitialState(Socket client) {
       ...
       for (Map.Entry<Integer, Integer> entry : stockPriceMap.entrySet()) {
           sendStockPriceToClient(client, entry.getKey(), entry.getValue());
       }
       
       // Transaction.javaから現在の保有状況を取得して送信
       Map<String, Integer> currentHoldings = requestCurrentHoldingsFromTransaction();
...
2. **イベントソーシング**:
   ...
   // 状態再構築
   public Portfolio reconstructPortfolio(int shareholderId, LocalTime until) {
       Portfolio portfolio = new Portfolio(shareholderId);
       List<TransactionEvent> events = eventStore.getEventsForShareholder(
           shareholderId, until);
...
1. **投資家タイプの導入**:
   ...
   private InvestorType assignInvestorType(ShareholderInfo shareholder) {
       ...
       if (age > 60) {
           return r.nextDouble() < 0.7 ? InvestorType.DIVIDEND_HUNTER : InvestorType.VALUE_INVESTOR;
       } else if (age < 30) {
           return r.nextDouble() < 0.6 ? InvestorType.GROWTH_SEEKER : InvestorType.DAY_TRADER;
       } else {
           // 中間年齢層はより多様
...
2. **市場条件に応じた行動変化**:
   ...
   class MarketCondition {
       private double overallSentiment;  // -1.0 (極度の悲観) 〜 1.0 (極度の楽観)
       private Map<Integer, Double> sectorSentiment; // セクターごとの市場感情
   ...
   private int determineQuantity(int shareholderId, int stockId, 
       ...
       switch (type) {
           ...
           case GROWTH_SEEKER:
               ...
               if (stock != null) {
                   baseProbability += market.getSectorSentiment(stock.getSector()) * 0.4;
               }
               break;
           // 他のタイプについても同様に調整
       }
   ...
   public void processStockSplit(int stockId, int ratio) {
       ...
       for (Portfolio portfolio : PortfolioManager.values()) {
           ...
           if (entry != null) {
               ...
               // ポートフォリオ更新
               portfolio.updateHoldingAfterSplit(stockId, newQuantity, newAvgCost);
           }
       }
