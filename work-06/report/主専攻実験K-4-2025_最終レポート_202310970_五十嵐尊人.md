...
- [7. 評価と検証・考察](#7-評価と検証考察)
  ...
  - [7.2 システムの限界と制約](#72-システムの限界と制約)
  - [7.3 リアルタイムデータ処理技術の比較](#73-リアルタイムデータ処理技術の比較)
- [8. 今後の展望](#8-今後の展望)
...
1. **React側のレンダリング速度の原因解明・改善**:
   - Webページでの表示速度に大きな問題があるので、改善方法を模索する。
   - 対策案としては、[先述](#712-パフォーマンス測定)した方法などがあるが、それ以外にも問題がある可能性が大いにあるため、再度`client-app`全体を見直す必要がある。
2. **初期状態同期メカニズム**:
